    ...
    namespace impl {
        ...
        parse_integer_base_prefix_for_detection(Range&& range)
        {
            ...
            }
            {
                ...
                if (auto r =
                        parse_integer_oct_base_prefix(range, zero_parsed)) {
        ...
        parse_integer_base_prefix(Range&& range, int base)
        {
            switch (base) {
                case 2:
                    // allow 0b/0B
                    return {
                        apply_opt(parse_integer_bin_base_prefix(range), range),
                        2, false};
                ...
                case 16:
                    // allow 0x/0X
                    return {
                        apply_opt(parse_integer_hex_base_prefix(range), range),
                        16, false};

                case 0:
                    // detect base
                    return parse_integer_base_prefix_for_detection(
                        SCN_FWD(range));
        ...
        auto parse_integer_prefix(Range range, int base) -> eof_expected<
            parse_integer_prefix_result<ranges::iterator_t<Range>>>
        {
            ...
            auto [digits_begin_it, parsed_base, parsed_zero] =
                parse_integer_base_prefix(
                    ranges::subrange{base_prefix_begin_it, ranges::end(range)},
                    base);
            ...
            for (; it != ranges::end(range); ++it) {
                if (*it == locale_options.thousands_sep) {
                    thsep_indices.push_back(
                        static_cast<char>(ranges_polyfill::pos_distance(
        ...
        public:
            ...
            read_default_with_base(Range&& range, T& value, int base)
            {
                SCN_TRY(prefix_result,
                        parse_integer_prefix(range, base)
                ...
                SCN_TRY(result_it, parse_integer_value(
                                       buf.view(), value, prefix_result.sign,
                                       prefix_result.parsed_base));
            ...
            {
                SCN_TRY(prefix_result,
                        parse_integer_prefix(range, specs.get_base(0))
                            .transform_error(make_eof_scan_error));
                ...
                if (SCN_LIKELY(!specs.localized)) {
                    ...
                    auto buf = make_contiguous_buffer(ranges::subrange{
                        prefix_result.iterator, after_digits_it});
                    SCN_TRY(result_it,
                            parse_integer_value(buf.view(), value,
                                                prefix_result.sign,
                                                prefix_result.parsed_base));
                ...
                SCN_TRY(nothsep_source_it,
                        parse_integer_value(nothsep_source_view, value,
                                            prefix_result.sign,
                                            prefix_result.parsed_base));

                return ranges_polyfill::batch_next(
                    prefix_result.iterator,
                    ranges::distance(nothsep_source_view.begin(),
                                     nothsep_source_it) +
