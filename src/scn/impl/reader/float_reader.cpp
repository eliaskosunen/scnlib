        ...
        namespace {
            ...
            protected:
                                                    ...
                                                    Strtod strtod_cb)
                {
                    ...
                    if (auto e =
                            this->check_error(chars_read, saved_errno, value);
                        SCN_UNLIKELY(!e)) {
                        return unexpected(e);
                    }
                                                     ...
                                                     T value) const
                {
                    ...
                    if (m_kind ==
                            float_reader_base::float_kind::hex_with_prefix &&
            ...
            public:
                ...
                {
                    return this->parse(
                        value, this->get_null_terminated_source(m_input),
                        generic_strtod);
                }
            ...
            private:
                ...
                {
                    if constexpr (std::is_same_v<CharT, char>) {
                        return strtod_impl_base<T>::generic_narrow_strtod(
                            str, str_end);
                    }
                    else {
                        return strtod_impl_base<T>::generic_wide_strtod(
                            str, str_end);
                    }
                }
            ...
            protected:
                    ...
                    std::string_view& input) const
                {
                    ...
                    if ((flags & std::chars_format::hex) !=
                        std::chars_format{}) {
                        if (m_kind ==
                            float_reader_base::float_kind::hex_without_prefix) {
                            return std::chars_format::hex;
                        }
                        else if (m_kind == float_reader_base::float_kind::
                                               hex_with_prefix) {
            ...
            public:
                ...
                scan_expected<std::ptrdiff_t> operator()(T& value) const
                {
                    ...
                    const auto result = std::from_chars(
                        input_view.data(),
                        input_view.data() + input_view.size(), value, *flags);
            ...
            template <typename CharT, typename T>
            scan_expected<std::ptrdiff_t> fast_float_fallback(
            ...
            struct fast_float_impl : fast_float_impl_base {
                ...
                scan_expected<std::ptrdiff_t> operator()(T& value) const
                {
                    if (m_kind ==
                        ...
                        return fast_float_fallback<CharT>(
                            {m_input, m_kind, m_options}, value);
                    }
            ...
            private:
                auto get_view() const
                {
                    ...
                    else if constexpr (get_encoding<CharT>() ==
                                       encoding::utf16) {
                ...
                T& value)
            {
                ...
                if constexpr (std::is_same_v<T, long double>) {
                    if constexpr (sizeof(double) == sizeof(long double)) {
                        ...
                        double tmp{};
                        auto ret = fast_float_impl<CharT, T>{data}(value);
                        value = tmp;
                        return ret;
                    }
                    else {
