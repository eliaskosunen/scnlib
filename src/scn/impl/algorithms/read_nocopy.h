...
namespace scn {
    ...
    namespace impl {
            ...
            Range&& range)
        {
            ...
            return ranges::next(SCN_FWD(range));
        }
        ...
        template <typename Range>
        scan_expected<ranges::borrowed_iterator_t<Range>>
        read_matching_code_unit(Range&& range, char ch)
        {
        ...
        template <typename Range>
        scan_expected<ranges::borrowed_iterator_t<Range>>
        read_matching_string_nocase(Range&& range, std::string_view str)
        {
            auto ascii_tolower = [](auto ch) {
                if (!is_ascii_char(ch)) {
                    return ch;
                }
                if (ch < 'A' || ch > 'Z') {
