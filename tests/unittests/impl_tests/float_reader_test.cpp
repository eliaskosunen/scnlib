...
#if SCN_HAS_STD_F16
        if constexpr (kind == float_kind::f16) {
            return MAKE_PAIR_RETURN(6.097555160522461e-5, F16);
        }
#endif
#if SCN_HAS_STD_BF16
        if constexpr (kind == float_kind::bf16) {
...
#if SCN_HAS_STD_BF16
        if constexpr (kind == float_kind::bf16) {
            return MAKE_PAIR_RETURN(0x1.fcp-127, BF16);
        }
#endif
        SCN_EXPECT(false);
        SCN_UNREACHABLE;
...
{
    EXPECT_TRUE(this->simple_default_test(
        "0x1.ap3", static_cast<typename TestFixture::float_type>(0x1.ap3)));
}
TYPED_TEST(FloatValueReaderTest, NegativeHex)
{
    EXPECT_TRUE(this->simple_default_test(
        "-0x1.bp2", static_cast<typename TestFixture::float_type>(-0x1.bp2)));
}
...
TYPED_TEST(FloatValueReaderTest, PresentationScientificValueScientific)
{
    auto [a, _, val] = this->simple_success_specs_test(
        "12.3e2", this->make_format_specs_with_presentation(
                      scn::detail::presentation_type::float_scientific));
    EXPECT_TRUE(a);
    EXPECT_TRUE(check_floating_eq(
        val, static_cast<typename TestFixture::float_type>(F_C(12.3e2))));
}
