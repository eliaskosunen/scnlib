...
namespace scn {
    ...
    namespace detail {
        ...
        class is_std_string_like {
            template <typename U>
            static auto check(U* p) -> decltype((void)p->find('a'),
                                                p->length(),
                                                (void)p->data(),
                                                int());
            template <typename>
        ...
        template <typename T>
        struct is_map<T, std::void_t<typename T::mapped_type>>
            : std::true_type {};
        ...
        template <typename T>
        struct is_set<T,
                      std::void_t<std::enable_if_t<!is_map<T>::value>,
        ...
        template <typename T,
                  typename CharT,
                  bool = is_tuple_like_impl<T>::value>
            ...
            static auto check(std::index_sequence<Is...>)
                -> decltype(check_impl(
                    ...
                    std::integer_sequence<
                        bool,
                        (has_scanner<std::tuple_element_t<Is, T>,
                                     CharT>)...>{}));
        ...
        struct range_mapper {
            ...
            template <typename T,
                      std::enable_if_t<has_scanner<T, CharT>>* = nullptr>
        ...
        using range_scanner_type =
            scanner<remove_cvref_t<decltype(range_mapper<CharT>().map(
                        SCN_DECLVAL(ElementT&)))>,
                    CharT>;
        ...
        {
            tuple_for_each(std::make_index_sequence<
        ...
        void add_element_to_range(Range& r, Element&& elem)
        {
            ...
            if constexpr (has_push_back<Range, elem_type>::value) {
                ...
                static_assert(dependent_false<Range>::value,
                              "Invalid range type");
            }
        }
        ...
        struct has_max_size<Range,
                            decltype(SCN_DECLVAL(const Range&).max_size())>
            : std::true_type {};
        ...
        private:
            ...
            scan_expected<typename Context::iterator> scan_inner_loop(
                Scan scan_cb,
                Context& ctx,
                Elem& elem,
                bool is_first) const
    ...
    public:
        ...
        constexpr scan_expected<typename ParseCtx::iterator> parse(
            ParseCtx& pctx)
        {
            return pctx.begin();
        }
    ...
    private:
        template <typename Context>
        scan_expected<typename Context::iterator> scan_for_each(
            Tuple& value,
            Context& ctx) const
        {
            scan_error err{};
    ...
    namespace detail {
        ...
        struct range_value_type_for_scanner_processor<
            ...
            using type =
                std::pair<std::remove_cv_t<First>, std::remove_cv_t<Second>>;
        };
        ...
        private:
            ...
            range_scanner<range_value_type_for_scanner<Range>, CharT>
                m_underlying;
        };
    }  // namespace detail
