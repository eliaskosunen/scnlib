...
namespace scn {
    ...
    namespace detail {
        ...
        inline constexpr bool is_span_compatible_sentinel =
            ranges_std::sized_sentinel_for<S, It> &&
            !std::is_convertible_v<S, size_t>;
        ...
        inline constexpr bool is_span_compatible_range =
            ...
            !is_std_array<detail::remove_cvref_t<R>> &&
            ranges::contiguous_range<R> && ranges::sized_range<R> &&
            (ranges::borrowed_range<R> || std::is_const_v<T>)&&std::
                is_convertible_v<
    ...
    public:
        ...
        template <
            ...
            std::enable_if_t<
                detail::is_span_compatible_iterator<Iterator, element_type> &&
                detail::is_span_compatible_sentinel<Sentinel, Iterator>>* =
                nullptr>
        ...
        template <size_t N>
        constexpr span(detail::type_identity_t<element_type> (&arr)[N])
            SCN_NOEXCEPT : m_ptr(arr),
                           m_end(arr + N)
        {
        }
        ...
        template <typename OtherT,
                  size_t N,
                  std::enable_if_t<std::is_convertible_v<OtherT (*)[],
                                                         element_type (*)[]>>* =
                      nullptr>
        ...
        template <typename Range,
                  std::enable_if_t<
                      detail::is_span_compatible_range<Range, element_type>>* =
                      nullptr>
    ...
    span(Iterator, End) -> span<
        std::remove_reference_t<ranges_std::iter_reference_t<Iterator>>>;
