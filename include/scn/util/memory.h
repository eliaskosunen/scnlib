...
// Workaround for MSVC _String_view_iterator
...
#if SCN_HAS_STD_SPAN
        template <typename T>
        struct is_wrapped_pointer_iterator<std::_Span_iterator<T>>
            : std::true_type {};
#endif
        ...
        struct is_wrapped_pointer_iterator<
            __gnu_cxx::__normal_iterator<ElementType*, Container>>
            : std::true_type {};
        ...
        struct can_make_address_from_iterator<
            It,
            std::enable_if_t<
                std::is_pointer_v<decltype(pointer_traits<It>::to_address(
